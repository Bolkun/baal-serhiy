...
def generate_excel_file(augment, dt_string, active_set, epoch, pickle_dir_path, df_lab_img): 
    ...
    uncertainties_var = df_lab_img.transpose()
    if augment == 1:
        uncertainties_var.columns = ['original', 'aug1', 'var']
    if augment == 2:   
        uncertainties_var.columns = ['original', 'aug1', 'aug2', 'var']

    uncertainties_var.to_excel(excel_path)
...
def main():
    ...
    dt_string = now.strftime("var_%d_%m_%Y_%Hx%M")
    csv_filename = "uncertainties/var_metrics_cifarnet_" + dt_string + "_.csv"
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        if len(pool) > 0:
            ...
            if probs is not None and len(probs) > 0:
                ...
                var_array = df_lab_img.var()

                # Save pickle file for every tenth epoch
                if (epoch+1) % 10 == 0:
                    pickle_dir_path, pickle_file_path = generate_pickle_file(dt_string, active_set, epoch, oracle_indices, var_array)
                    #mypickle = pd.read_pickle(pickle_file_path)

                df_lab_img = pd.DataFrame(np.vstack([matrix, var_array]))
                ...
                # 3. Map var uncertainties to uncertainty array
                if hyperparams["augment"] == 1:
                    for i in range(len(uncertainty)):
                        uncertainty[i] = var_array[i % (len(pool)/2-1)]
                if hyperparams["augment"] == 2:
                    for i in range(len(uncertainty)):
                        uncertainty[i] = var_array[i % (len(pool)/3-1)]
